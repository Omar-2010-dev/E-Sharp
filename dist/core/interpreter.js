...
class Interpreter {
    constructor() {
        ...
        this.steps = 0;
        this.maxSteps = 5000000; // زودنا الحد الأقصى عشان الألعاب التقيلة
        this.loopDepth = 0; // متغير جديد عشان نعرف إحنا جوه loop ولا لأ
    ...
    builtins() {
        // 🎮 Raylib Integration
        let r; // هنحمل المكتبة لما نحتاجها
        // دالة مساعدة لتحويل النص للون Raylib
        const getColor = (name) => {
            if (!r)
                return { r: 255, g: 255, b: 255, a: 255 }; // Default White
        ...
        const b = {
            ...
            toNum: (v) => {
                const n = Number(v);
                if (isNaN(n))
                    throw new ESharpError(`Can't convert '${v}' to number`);
                return n;
            },
            toStr: (v) => this.str(v),
            // 🧮 Math
            ...
            randomize: (arr) => {
                if (!Array.isArray(arr))
                    throw new ESharpError('randomize needs a list');
                const shuffled = [...arr];
            ...
            pick: (arr) => {
                if (!Array.isArray(arr))
                    throw new ESharpError('pick needs a list');
            ...
            sort: (arr) => {
                if (!Array.isArray(arr))
                    throw new ESharpError('sort needs a list');
                return [...arr].sort((a, b) => Number(a) - Number(b));
            },
            unique: (arr) => {
                if (!Array.isArray(arr))
                    throw new ESharpError('unique needs a list');
                return [...new Set(arr)];
            },
            // دالة جديدة بتمسح أول عنصر بيقابله قيمته زي القيمة اللي باعتها
            remove: (arr, val) => {
                if (!Array.isArray(arr))
                    throw new ESharpError('remove needs a list');
            ...
            flat: (arr) => {
                if (!Array.isArray(arr))
                    throw new ESharpError('flat needs a list');
                return arr.flat();
            },
            count: (arr, val) => {
                if (!Array.isArray(arr))
                    throw new ESharpError('count needs a list');
                return arr.filter((x) => x === val).length;
            },
            insert: (arr, i, val) => {
                if (!Array.isArray(arr))
                    throw new ESharpError('insert needs a list');
            ...
            zip: (a, b) => {
                if (!Array.isArray(a) || !Array.isArray(b))
                    throw new ESharpError('zip needs two lists');
            ...
            // 🔑 Dict functions
            keys: (o) => o && typeof o === 'object' && !Array.isArray(o)
                ? Object.keys(o)
                : [],
            values: (o) => o && typeof o === 'object' && !Array.isArray(o)
                ? Object.values(o)
                : [],
            // 🎨 Fun & Emoji
            emoji: (name) => {
            ...
            measureText: (text, fontSize) => {
                if (!r)
                    throw new ESharpError('Raylib not initialized. Call showWindow() first.');
    ...
    format(v) {
        ...
        if (v === null || v === undefined)
            return '\x1b[90mnothing\x1b[0m';
        // Boolean -> لون بنفسجي فاتح (Bright Magenta)
        if (typeof v === 'boolean')
            return v ? '\x1b[95mtrue\x1b[0m' : '\x1b[95mfalse\x1b[0m';
        // Number -> لون أصفر فاتح (Bright Yellow)
        if (typeof v === 'number')
            return `\x1b[93m${v}\x1b[0m`;
        // String -> لون أخضر فاتح (Bright Green)
        if (typeof v === 'string')
            return `\x1b[92m${v}\x1b[0m`;
        // List -> تلوين العناصر اللي جواها بشكل متكرر
        if (Array.isArray(v))
            return '[' + v.map((x) => this.format(x)).join(', ') + ']';
        // Object -> تلوين القيم
        if (typeof v === 'object') {
            const o = v;
            // الكلاسات والدوال -> لون سماوي
            if (o.__type === 'instance')
                return `\x1b[36m<${v.className}>\x1b[0m`;
            if (o.__type === 'function')
                return `\x1b[36m<func ${v.name}>\x1b[0m`;
            if (o.__type === 'builtin')
                return `\x1b[36m<${o.name}>\x1b[0m`;
            return ('{' +
                Object.entries(o)
    ...
    str(v) {
        ...
        if (typeof v === 'boolean')
            return v ? 'true' : 'false';
        if (Array.isArray(v))
            return '[' + v.map((x) => this.str(x)).join(', ') + ']';
        if (typeof v === 'object') {
            const o = v;
            if (o.__type === 'instance')
                return `<${v.className}>`;
    ...
    async run(source) {
        ...
        try {
            ...
            await this.block(ast.body, this.env);
        }
        catch (e) {
    ...
    check() {
        if (++this.steps > this.maxSteps)
            throw new ESharpError('Too many steps (infinite loop?)');
    }
    // تنفيذ البلوك بقى Async
    async block(stmts, env) {
        let r = null;
    ...
    async exec(node, env) {
        ...
        try {
            ...
            if (e instanceof ESharpError && !e.line)
                e.line = node.line;
            throw e;
        }
    }
    async execSafe(node, env) {
        switch (node.type) {
            case 'Program':
            ...
            case 'MethodCall': {
                ...
                if (Array.isArray(obj))
                    return this.arrMethod(obj, node.method, args);
                if (typeof obj === 'string')
                    return this.strMethod(obj, node.method, args);
            ...
            case 'PropertyAccess': {
                ...
                if (Array.isArray(obj) && node.property === 'length')
                    return obj.length;
                if (typeof obj === 'string' && node.property === 'length')
                    return obj.length;
            ...
            case 'NewExpr': {
                ...
                if (!cls)
                    throw new ESharpError(`Class '${node.className}' not found`);
            ...
            case 'BinaryExpr':
                return this.binary(node.op, await this.exec(node.left, env), await this.exec(node.right, env));
            case 'UnaryExpr': {
    ...
    binary(op, l, r) {
        ...
        if (op === '/') {
            if (Number(r) === 0)
                throw new ESharpError('Division by zero!');
            return Number(l) / Number(r);
        }
        if (op === '%')
            return Number(l) % Number(r);
    ...
    findMethod(cls, name) {
        ...
        if (cls.parent) {
            ...
            if (p)
                return this.findMethod(p, name);
        }
        return null;
    }
    async instMethod(inst, method, args, env) {
        const cls = this.classes.get(inst.className);
        if (!cls)
            throw new ESharpError(`Class '${inst.className}' not found`);
        const m = this.findMethod(cls, method);
        if (!m)
            throw new ESharpError(`No method '${method}' on '${inst.className}'`);
        const e = new Environment(env);
        e.define('this', inst);
    ...
    strMethod(s, m, args) {
        ...
        if (m === 'trim')
            return s.trim();
        if (m === 'split')
            return s.split(String(args[0] ?? ' '));
        if (m === 'has')
            return s.includes(String(args[0])); // has هي نفسها includes
        if (m === 'startsWith')
